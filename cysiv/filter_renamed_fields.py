...
def get_renamed_fields_per_data_source(csv_file, data_sources):
    folder_name = f"data_sources_renamed_fields_{str(uuid.uuid4())[0:8]}"
    ...
    for data_source in data_sources:
        with open(f"{folder_name}/{data_source}.csv", mode='w') as ds_ec:
            ...
            with open(csv_file) as csv_f:
                ...
                for row in csv_reader:
                    if line_count == 0:
                        file_writer.writerow(["Count", row[0], row[2], row[3]])
                        line_count += 1
                    else:
                        if data_source in row[4] or data_source in row[5]:
                            if row[2] and "event_context" not in row[2]:
                                file_writer.writerow([line_count, row[0].replace("(new field)", ""), row[2], row[3]])
                                line_count += 1
...
if __name__ == "__main__":
    ...
    get_renamed_fields_per_data_source(file, data_sources)
